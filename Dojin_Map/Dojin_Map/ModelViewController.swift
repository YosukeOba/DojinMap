...
class ModelViewController: UIViewController, UITextFieldDelegate, UIPickerViewDelegate, UIPickerViewDataSource, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    //let csvPath = Bundle.main.path(forResource: "circle", ofType: "csv")
        ...
        if(String(circleTextField.text!) != "" && String(placeWordTextField.text!) != "" && String(placeNumTextField.text!) != "" && Int(placeNumTextField.text!)! != 0){
            ...
            if(!isTwitterID(id: twitterTextField.text!)){
                ...
                if ImgImageView.image == nil {
                    print("NoImage")
                    saveImage(image: NoImage!, filename: primaryKey/*String(twitterTextField.text!.trimmingCharacters(in: .whitespaces))*/)
                }else{
    ...
    func textFieldDidBeginEditing(_ textField: UITextField) {
        ...
        if textField == twitterTextField || textField == memoTextField{
            //selectedTextField = String(textField.accessibilityIdentifier!)
            // Notificationを設定する
            switch textField {
...
//        let letter: String = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
//        var outputString = ""
//
//        for _ in 0..<length {
//            let randLetter = String(letter[letter.index(letter.startIndex, offsetBy: Int.random(in: 0 ... letter.count))])
//            outputString += randLetter
//        }
//        print(outputString)
//        return outputString
//    }
